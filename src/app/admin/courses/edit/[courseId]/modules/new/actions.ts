...
export async function createModule(formData: FormData) {
  ...
  const redirect_to =
    ...
    (courseId ? `/admin/courses/edit/${courseId}#modules` : "/admin/courses");
  ...
  if (!(isAdmin(role) || course.instructor_id === user.id)) {
    throw new Error("Forbidden");
  }

  // Compute next ordering + clamp requested position
  ...
  // If inserting in the middle, shift existing rows down (desc to avoid collisions)
